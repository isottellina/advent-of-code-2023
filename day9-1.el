  ...
  (cl-labels ((list-of-differences (seq)
              ...
              (next-value (seq
                ...
                (if (= (length distinct-seq) 1)
                    (car distinct-seq)
                  (+ (-last-item seq) (next-value (list-of-differences seq))))))
    (next-value seq)))

(with-current-buffer "input.txt"
  (goto-char (point-min))
  (cl-loop until (and (eolp) (bolp))
           sum (day9/get-next-value (cl-loop while (re-search-forward "-?[[:digit:]]+" (line-end-position) t)
