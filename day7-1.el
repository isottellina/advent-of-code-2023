...
(defun card-hand< (card-hand1 card-hand2)
  (let ((card1-primary-rank (card-primary-rank card-hand1))
        (card2-primary-rank (card-primary-rank card-hand2)))
    (cond ((/= card1-primary-rank card2-primary-rank) (< card1-primary-rank card2-primary-rank))
          (t (card-secondary-rank< card-hand1 card-hand2)))))

(card-hand< "QQQJA" "KK677")
...
(with-current-buffer "input.txt"
  ...
  (let ((hands-bids (cl-loop until (and (bolp) (eolp))
    ...
    (-sum (seq-map-indexed (pcase-lambda (`(,_ . ,bid) idx) (* bid (+ idx 1)))
                           (seq-sort (pcase-lambda (`(,card-hand1 . _) `(,card-hand2 . _))
                                       (card-hand< card-hand1 card-hand2))
                                     hands-bids)))))
